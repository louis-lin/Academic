function record = Static(P, MatData, MatState, algorithm_type , max_iter)
    record.P = P; % Saves the applied forces
    record.R = []; % Init internal force
    record.P_iter = []; % Record of R at every iteration
    record.U_iter = [];% Record of U at every iteration
    record.iter = []; % Record iterations per run
    record.MatData = MatData; % Saves the material data
    record.MatState = MatState; % Saves initial Material State
    ...
    for n = 1:numel(P)-1% Loop over load steps
        ...
        while ( j <= max_iter && conv == 0) % Loop over Newton-Raphson iterations while not converged
            ...
            R = MatData.A*MatState.Pres.sig; % Calculate the internal resisting force
            Unb = P(n+1)-R; % Calculate the unbalance force 
            record.Unb(n,j) = abs(Unb); % Record the unbalance force
            
            if (j > 1)  
                record.U_iter = [record.U_iter,U_conv]; % Record the unbalance force path
                record.P_iter = [record.P_iter,R];% Record the unbalance force path
            end
           % Converged branch
            if (abs(Unb) < 1.e-5) % norm convergence criteria
                ...
                record.iter(n) =  j; % Record the iterations
            else
                ...
                record.U_iter = [record.U_iter,U_conv]; % Record the unbalance force path
                record.P_iter = [record.P_iter,P(n+1)];% Record the unbalance force path
            end
        end
    end
end