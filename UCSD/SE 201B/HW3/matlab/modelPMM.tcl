...
section Fiber $secTag -GJ $Ubig {
	...
	# -------------------------------------------------------------------------------------
	# Cover concrete
	patch rect $matTagConcCover $fiberA $fiberB [expr $cover - $y1] [expr -$z1] [expr $y1 - $cover] [expr $cover - $z1]
	patch rect $matTagConcCover $fiberA $fiberB [expr $cover - $y1] [expr $z1 - $cover] [expr $y1 - $cover] [expr $z1]
	patch rect $matTagConcCover $fiberB [expr $fiberA+2*$fiberB] [expr -$y1] [expr -$z1] [expr $cover - $y1] [expr $z1]
	patch rect $matTagConcCover $fiberB [expr $fiberA+2*$fiberB] [expr $y1 - $cover] [expr -$z1] [expr $y1] [expr $z1]
	# Core concrete
	patch rect $matTagConcCore $fiberC $fiberC [expr $cover - $y1] [expr $cover - $z1] [expr $y1 - $cover] [expr $z1 - $cover]
	# -------------------------------------------------------------------------------------
...
# puts $modelExportFileID "section Fiber $secTag -GJ $Ubig {
	...
	# patch rect $matTagConcCover $fiberA $fiberB [expr $cover - $y1] [expr -$z1] [expr $y1 - $cover] [expr $cover - $z1]
	# patch rect $matTagConcCover $fiberA $fiberB [expr $cover - $y1] [expr $z1 - $cover] [expr $y1 - $cover] [expr $z1]
	# patch rect $matTagConcCover $fiberB $fiberA [expr -$y1] [expr -$z1] [expr $cover - $y1] [expr $z1]
	# patch rect $matTagConcCover $fiberB $fiberA [expr $y1 - $cover] [expr -$z1] [expr $y1] [expr $z1]
	# # Core concrete
	# patch rect $matTagConcCore $fiberC $fiberC [expr $cover - $y1] [expr $cover - $z1] [expr $y1 - $cover] [expr $z1 - $cover]
	# # -------------------------------------------------------------------------------------
	# # Create straight layers
